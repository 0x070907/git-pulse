...
**Deploy:** Render  
**Data source:** GitHub REST API (authenticated so 5000 req/hour)
...
2. Enter a username (e.g. `torvalds`, `octocat`, your own)
3. Click **Execute** - the full analysis returns as JSON

#### Note :
- The response includes profile card, a stats row, a profile strength score with actionable tips, language breakdown by percentage, repo quality scores, a collaboration badge, an activity heatmap, and a recent event feed, all of which are derived from three parallel API calls.
...
- `theme` can be "dark", "default", "rose", "blue_navy", "github_dark" or "midnight-purple" which controls the styling of the generated README cards.This is made available as a dropdown in the frontend UI.
- The response contains a `markdown` field which contains the full generated `README.md` as a raw string. It may look messy or hard to read in its raw form (lots of `\n`,`\"` escape sequences, badge URLs, etc.) as this is expected. The frontend will render it correctly into a clean, formatted preview once it's live.
